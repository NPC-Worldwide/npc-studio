...
const LatexViewer = ({
  ...
  const compile = useCallback(async () => {
    ...
    setError(null);
    try {
      // Expectation: window.api.compileLatex(filePath) returns { pdfPath, log, error? }
      const res = await window.api.compileLatex(filePath);
      if (res?.error) throw new Error(res.error);
      if (res?.log) setCompileLog(res.log);
      // Open immediately after successful compile
      if (res?.pdfPath) {
        await window.api.openFile(res.pdfPath);
      } else {
        // Fallback to conventional .pdf next to .tex
